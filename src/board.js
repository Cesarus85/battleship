...
export class Board extends THREE.Group {
  constructor({ size = 1.0, divisions = 10 } = {}) {
    ...
    this.ghostGroup = new THREE.Group(); this.add(this.ghostGroup);
    this.shipsGroup = new THREE.Group(); this.add(this.shipsGroup);
    this.shotsGroup = new THREE.Group(); this.add(this.shotsGroup);
  }
  ...
  // Unterstützt 'H'/'V' und 'horizontal'/'vertical'
  cellsForShip(anchor, length, orientation) {
    const ori = String(orientation ?? '').toLowerCase();
    const isH = (orientation === 'H') || (ori === 'h') || (ori === 'horizontal') || (orientation === 0);
    const isV = (orientation === 'V') || (ori === 'v') || (ori === 'vertical')   || (orientation === 1);
    const out = [];
    for (let k = 0; k < Number(length || 1); k++) {
      const i = anchor.i + (isH ? k : 0);
      const j = anchor.j + (isV ? k : 0);
      out.push({ i, j });
    }
    return out;
  }

  cellCenterLocal(i, j) { return this._cellToLocal(i, j); }
}
